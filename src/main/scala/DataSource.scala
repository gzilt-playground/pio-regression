  ...
  : Seq[(TrainingData, EmptyEvaluationInfo, RDD[(Query, ActualResult)])] = {
    ...
    (0 until evalK).map { idx =>
      ...
      (
        ...
        testingPoints.map {
          p => (Query(p.features(0).toInt, p.features(1).toInt, p.features(2), p.features(3), p.features(4), p.features(5), p.features(6).toInt), ActualResult(p.label))
        }
      )
    }
  }
